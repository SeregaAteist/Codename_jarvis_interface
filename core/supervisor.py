...
class Supervisor(BaseAgent):
    ...
    async def _run(self, capability: str, payload) -> dict:
        ...
        try:
            ...
            await bus.emit(
                "task.completed",
                {"capability": capability, "ok": True, "result": result},
            )
    ...
    async def _make_plan(self, capability: str, payload) -> str:
        return (
            f"📋 План (RCE-уровень, требуется подтверждение):\n"
    ...
    async def dispatch_task(self, capability: str, payload=None) -> dict:
        ...
        approved = await self._approve(
            plan, {"capability": capability, "payload": payload}
        )
        if not approved:
...
def register_supervisor(approve_callback: ApproveCb | None = None) -> Supervisor:
    ...
    supervisor.wire_bus()
    logger.info(
        "[captain] зарегистрирован, approve=%s", "on" if supervisor._approve else "off"
    )
    return supervisor
