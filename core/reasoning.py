...
_INTENTS: list[tuple[str, re.Pattern]] = [
    (
        "morning",
        re.compile(
            r"\b(доброе утро|добрый день|добрый вечер|как дела|как настроение|утренний|что нового|привет)\b",
            re.I,
        ),
    ),
    (
        "mac",
        re.compile(
            r"\b(открой|открыть|запусти|запустить|закрой|громкость|заблокируй|блокировка|уведомление|яркость|скриншот|выполни команду)\b",
            re.I,
        ),
    ),
    (
        "ollama",
        re.compile(
            r"\b(быстро|локально|офлайн|без интернета|локальная модель)\b", re.I
        ),
    ),
    ...
    (
        "news",
        re.compile(
            r"\b(новости|что происходит|расскажи новости|последние события|заголовки)\b",
            re.I,
        ),
    ),
    (
        "memory",
        re.compile(
            r"\b(запомни|запиши|заметку|заметка|что ты знаешь|что знаешь обо мне|мои заметки|что записал|статистика|сколько мы|сколько раз)\b",
            re.I,
        ),
    ),
    (
        "search",
        re.compile(
            r"\b(найди|поищи|что такое|кто такой|погугли|поищи|найти|нагугли)\b", re.I
        ),
    ),
    (
        "price",
        re.compile(r"\b(цена|сколько стоит|стоимость|почём|купить|цены на)\b", re.I),
    ),
    (
        "open_url",
        re.compile(r"\b(открой сайт|перейди на|открой https?://|открой www\.)\b", re.I),
    ),
