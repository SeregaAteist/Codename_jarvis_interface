...
from connectors.voice import speak as _speak_raw
from connectors.voice import stop as _stop_raw
...
from core.security import log_auth, log_input_violation, log_rate_limit
...
if not _TOKEN:
    ...
    print("[auth] standalone mode — token generated (not logged for security)")
...
class JarvisHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        if not self._rate_ok():
            self._deny(429, "rate limit exceeded")
            return
        ...
        if path in ("/", "/hud"):
            ...
            self._json(
                {"paused": listener.is_paused(), "disabled": listener.is_disabled()}
            )
        ...
        elif path == "/api/rafail/status":
            self._handle_api_rafail_status()
        elif path == "/api/kommo/status":
            self._handle_api_kommo_status()
        elif path == "/api/calls/recent":
            self._handle_api_calls_recent()
        else:
    ...
    def do_POST(self):  # noqa: N802
        ...
        if path == "/ask":
            ...
            broadcast(
                {"type": "telemetry", "text": f"USR {text[:40]} → AGT {resp.agent}"}
            )
            speak(resp.text)
            ...
            if enabled is True:
                ...
                self._json({"error": "enabled must be bool"}, 400)
                return
        ...
        try:
            if action == "open_app":
                ...
                result = mc.show_notification(
                    params.get("title", "JARVIS"), _clean(params.get("message", ""))
                )
    ...
    def _handle_browse(self, body: dict):
        ...
        broadcast(
            {"type": "telemetry", "text": f"BROWSER · {action.upper()} · {query[:40]}"}
        )
        ...
        broadcast(
            {"type": "telemetry", "text": f"BROWSER · COMPLETE · {len(result)} chars"}
        )
        self._json({"result": result, "action": action, "query": query})

    # ── API: agents/status (launchctl) ───────────────────────────────────────
    ...
    def _handle_api_agents_status(self):
        try:
            ...
            self._json({"services": services, "total": len(services)})
        except Exception as e:
            self._json({"error": str(e)}, 500)

    # ── API: rafail/status ────────────────────────────────────────────────────

    def _handle_api_rafail_status(self):
        try:
            ...
            with connect() as c:
                ...
                uploaded = c.execute(
                    "SELECT COUNT(*) FROM processed WHERE status='uploaded'"
                ).fetchone()[0]
    ...
    def _handle_api_kommo_status(self):
        import asyncio
        ...
        async def _check():
            ...
            client = KommoClient()
            leads = await client.get_leads(limit=1)
            return {"status": "ok", "leads_count": len(leads)}
    ...
    def _handle_api_calls_recent(self):
        from pathlib import Path

        calls_dir = Path.home() / "Projects/jarvis/data/calls"
        ...
        self._json(
            {
                "calls": [
                    {"id": f.stem, "size_kb": f.stat().st_size // 1024}
                    for f in calls[:10]
                ]
            }
        )
