...
def _mac_dispatch(text: str) -> str | None:
    from system.mac_control import open_app, set_volume, lock_screen, show_notification
    t = text.strip()
    m = _APP_PAT.search(t)
    if m:
...
def _memory_dispatch(text: str) -> str:
    from system.preferences import learn, get_all_learned, add_note, get_notes
    from system.memory import get_stats_summary
    t = text.strip()
...
def _browser_dispatch(text: str, intent: str, ollama) -> str:
    ...
    from system.agents.browser_agent import smart_search, get_price, fetch_page, open_url_in_browser
