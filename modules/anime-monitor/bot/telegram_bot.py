...
- Поиск онлайн: верни "ACTION:SEARCH_ONLINE:{название}"
- Что выходит на этой неделе / сезон: верни "ACTION:WEEKLY"
...
class AnimeBot(JarvisBot):
    ...
    async def _search_online(self, title: str) -> str:
        """Поиск аниме через Jikan API если нет в локальном каталоге."""
        import httpx
        ...
        for anime in results[:3]:
            lines.append(
                ...
                f"  {anime.get('episodes', '?')} эп | "
                f"⭐{anime.get('score', '?')} | "
                f"{anime.get('status', '')}"
            )
        return f"🔍 Результаты поиска '{title}':\n\n" + "\n".join(lines)

    async def _get_weekly_anime(self) -> str:
        """Аниме текущего сезона через Jikan, с фильтром по жанрам вотчлиста."""
        import httpx

        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.get(f"{cfg.JIKAN_URL}/seasons/now", params={"limit": 25})
            data = r.json()

        items = data.get("data", [])
        if not items:
            return "Не вдалося отримати дані про поточний сезон."

        # Жанры из вотчлиста для фильтрации
        watchlist_titles = {w["title"].lower() for w in get_watchlist()}

        lines = [f"📅 Виходить цього сезону ({len(items)} тайтлів):\n"]
        shown = 0
        for anime in items[:20]:
            title = anime.get("title", "")
            score = anime.get("score") or "?"
            episodes = anime.get("episodes") or "?"
            status = anime.get("status", "")
            # Помечаем тайтлы из вотчлиста
            mark = "⭐ " if title.lower() in watchlist_titles else "▶️ "
            lines.append(f"{mark}<b>{title}</b> | {episodes} еп | ⭐{score} | {status}")
            shown += 1
            if shown >= 10:
                break
    ...
    async def _build_answer(self, text: str) -> str:
        ...
        if "ACTION:SEARCH_ONLINE:" in answer:
            query = answer.split("ACTION:SEARCH_ONLINE:")[1].split("\n")[0].strip()
            return await self._search_online(query)
        if "ACTION:WEEKLY" in answer:
            return await self._get_weekly_anime()
