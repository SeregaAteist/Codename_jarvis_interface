...
async def _send(chat_id: int, text: str, thread_id: int | None = None,
                token: str | None = None) -> None:
    payload: dict = {"chat_id": chat_id, "text": text,
                     "parse_mode": "HTML", "disable_web_page_preview": True}
    if thread_id:
        payload["message_thread_id"] = thread_id
    _token = token or os.getenv("JARVIS_NOTIFY_BOT_TOKEN") or TG_TOKEN
    async with httpx.AsyncClient(timeout=10) as c:
        r = await c.post(f"https://api.telegram.org/bot{_token}/sendMessage", json=payload)
